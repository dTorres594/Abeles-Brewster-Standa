...
{                    //Inicio del void loop.
  ...
  if (Serial.available()) {  //Solo entra cuando hay una interrupcion en el puerto.
    ...
    motor = bi[0];
    direccion = bi[1];
    pasos_MSB=bi[2]<<8;             //Se le el byte más significativo y se convierte a numero para los pasos del bipolar.
    ...
    Serial.println(pasos_totales);         // #DTA: print total unipolar steps to port    
    ...
    */          
    ...
    }  // Fin del ciclo por número de pasos                                                
