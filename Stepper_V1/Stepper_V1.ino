...
const int endStop1Pin = 18;
const int endStop2Pin = 19;
...
unsigned char dataAtPort[] = {0,  //[0] Shield action. 0: Init; 1: Move; Miscellaneous commands
                              ...
                              0,    //[3] On init: rpm motor1; on move: total steps MSB motor1
                              0,    //[4] On init: rpm motor2; on move: total steps LSB motor1
                              0,    //[5] On move: total steps MSB motor2
                              0,    //[6] On move: total steps LSB motor2
                              0,    //[7] On move: step style motor 1
                              0
                             };   //[8] On move: step style motor 2
...
void loop() {
  ...
  if (Serial.available() > 0) { // Only run routines if there's information at the port
    ...
    delay(50);
    str_len = dataAtPortStr.length() + 1;
    dataAtPortStr.toCharArray(dataAtPort, str_len);
    //Serial.println(str_len);    
    /*Serial.println("Read: " + dataAtPortStr);
    ...
    if (dataAtPort[0] == INIT) { // Initialization routine
      ...
      delay_motor1 = motor1->setSpeed(dataAtPort[3]);
      delay_motor2 = motor2->setSpeed(dataAtPort[4]);
      ...
      //Serial.print("Milliseconds: " + (String)wait_ms + "\n");
      ...
      Serial.println("Sen" + (String)sensor_voltage);
    }
      ...
      for (int current_step = 0; current_step < maxSteps; current_step++) { // Start moving

        if (current_step >= 500) { Home = 0; }  // Release Home flag
        ...
        if ((current_step % factor1) == 0 ||
          ...
          if (Serial.available()) { // See if new commands are available at the port
            ...
            if (instruction[0] == STOP) { // Check if a stop has been received
              ...
              Serial.println(positionStr + "Sen" + (String)sensor_voltage + 
                            "ES" + (String)ES1 + "," + (String)ES2);
            }
          } // End of Serial.available within for loop.

          if ((current_step % factor1) == 0 && !ES1) { // Motor 1 takes a step
            if (Home && directionMotor1 == FORWARD){   // Do not move forward if at home
              Serial.println("Home");
              break;
            } else{
          ...
          if ((current_step % factor2) == 0 && !ES2) { // Motor 2 takes a step
            if (Home && directionMotor2 == FORWARD){   // Do not move forward if at home
              Serial.println("Home");
              break;
            }else{
