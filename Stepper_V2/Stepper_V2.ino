...
const int sensorPin = A10;  //Change to A0 on Uno
    ...
    if (dataAtPort[0] == INIT) { // Initialization routine
      ...
      for (int current_step = 1; current_step <= maxSteps; current_step++) { // Start moving
        ...
        if ((current_step % factor1) == 0 ||
          ...
          if (totalStepsMotor1 != 0 && (current_step % factor1) == 0 && !ES1) { // Motor 1 takes a step
          ...
          if (totalStepsMotor2 != 0 && (current_step % factor2) == 0 && !ES2) { // Motor 2 takes a step
